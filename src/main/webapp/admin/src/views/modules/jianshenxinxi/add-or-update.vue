	...
	<div class="addEdit-block" style="width: 100%;">
		...
		>
			<template >
				...
				<el-form-item :style='{"margin":"0 0 20px 0"}' class="input" v-if="type!='info'"  label="健身类型" prop="jianshenleixing">
					<el-input v-model="ruleForm.jianshenleixing" placeholder="健身类型" clearable  :readonly="ro.jianshenleixing"></el-input>
				</el-form-item>
				<el-form-item :style='{"margin":"0 0 20px 0"}' v-else class="input" label="健身类型" prop="jianshenleixing">
					<el-input v-model="ruleForm.jianshenleixing" placeholder="健身类型" readonly></el-input>
				</el-form-item>
				...
				<el-form-item :style='{"margin":"0 0 20px 0"}' class="upload" v-if="type!='info'&& !ro.shipinjiaoxue" label="视频教学" prop="shipinjiaoxue">
					<file-upload
						tip="点击上传视频教学"
						action="file/upload"
						:limit="1"
						:multiple="true"
						:fileUrls="ruleForm.shipinjiaoxue?ruleForm.shipinjiaoxue:''"
						@change="shipinjiaoxueUploadChange"
					></file-upload>
				</el-form-item> 
				<el-form-item :style='{"margin":"0 0 20px 0"}' v-else-if="ruleForm.shipinjiaoxue" label="视频教学" prop="shipinjiaoxue">
					<el-button :style='{"border":"1px solid #3ca512","cursor":"pointer","padding":"0 35px","margin":"0 20px 0 0","outline":"none","color":"rgba(255, 255, 255, 1)","borderRadius":"4px","background":"url(http://codegen.caihongy.cn/20221011/ca1c191554d24b108bc94f4a2046d636.png) #41b314 no-repeat 15px 12px","width":"auto","lineHeight":"40px","fontSize":"14px","height":"40px"}' type="text" size="small" @click="download($base.url+ruleForm.shipinjiaoxue)">预览</el-button>
				</el-form-item>
				<el-form-item :style='{"margin":"0 0 20px 0"}' v-else-if="!ruleForm.shipinjiaoxue" label="视频教学" prop="shipinjiaoxue">
				...
				<el-form-item :style='{"margin":"0 0 20px 0"}' class="textarea" v-if="type!='info'" label="健身内容" prop="jianshenneirong">
					<el-input
					  style="min-width: 200px; max-width: 600px;"
					  type="textarea"
					  :rows="8"
					  placeholder="健身内容"
					  v-model="ruleForm.jianshenneirong" >
					</el-input>
				</el-form-item>
				<el-form-item :style='{"margin":"0 0 20px 0"}' v-else-if="ruleForm.jianshenneirong" label="健身内容" prop="jianshenneirong">
					<span :style='{"fontSize":"14px","lineHeight":"40px","color":"#333","fontWeight":"500","display":"inline-block"}'>{{ruleForm.jianshenneirong}}</span>
				</el-form-item>
...
export default {
	data() {
		...
		return {
			...
			rules: {
				...
				jianshenleixing: [
					{ required: true, message: '健身类型不能为空', trigger: 'blur' },
				],
				tupian: [
				],
				shipinjiaoxue: [
		...
		init(id,type) {
			...
			if(this.type=='info'||this.type=='else'){
				...
				for (var o in obj){
						...
						if(o=='shipinjiaoxue'){
							this.ruleForm.shipinjiaoxue = obj[o];
							this.ro.shipinjiaoxue = true;
							continue;
						}
						if(o=='jianshenneirong'){
							this.ruleForm.jianshenneirong = obj[o];
							this.ro.jianshenneirong = true;
							continue;
						}
						if(o=='fabushijian'){
	...
	if(this.ruleForm.shipinjiaoxue!=null) {
		this.ruleForm.shipinjiaoxue = this.ruleForm.shipinjiaoxue.replace(new RegExp(this.$base.url,"g"),"");
	}
...
var objcross = this.$storage.getObj('crossObj');
		 ...
		 if(crossrefid && crossuserid) {
			...
			this.$http({ 
				...
				if (data && data.code === 0) { 
					 ...
					 this.$http({
					   url: `jianshenxinxi/${!this.ruleForm.id ? "save" : "update"}`,
					   method: "post",
					   data: this.ruleForm
					 }).then(({ data }) => {
					   if (data && data.code === 0) {
						 ...
						 this.parent.jianshenxinxiCrossAddOrUpdateFlag = false;
						 this.parent.search();
    ...
    shipinjiaoxueUploadChange(fileUrls) {
	    this.ruleForm.shipinjiaoxue = fileUrls;
    },
  }
};
</script>
